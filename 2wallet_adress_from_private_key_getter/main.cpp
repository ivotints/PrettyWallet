...
int heuristic_repeated_pairs(const std::string &addr)
{
    if (addr.size() < 6)
        return 0;
    bool pairs = true;
    for (size_t i = 2; i < addr.size() - 1; i += 2)
    {
        if (addr[i] != addr[i + 1])
        {
            pairs = false;
...
int heuristic_vanity_words(const std::string &addr)
{
    ...
    for (const auto &van : vanities)
    {
        ...
        if (pos != std::string::npos)
        {
            ...
            else if (pos + van.size() == hex_part.size())
...
{
    int score = 0;
    size_t len = addr.size() - 2;
    for (size_t i = 0; i < len - 1; ++i)
    {
        int val1 = std::tolower(addr[2 + i]) - '0';
        if (val1 > 9)
            val1 = val1 - 'a' + 10;
        int val2 = std::tolower(addr[2 + i + 1]) - '0';
        if (val2 > 9)
            val2 = val2 - 'a' + 10;
        if (val2 == val1 + 1)
        {
...
int heuristic_long_runs(const std::string &addr)
{
    ...
    if (max_run_start == 2 || max_run_start + max_run == addr.size())
    {
        points *= 2;
    }
    return points;
